...
script.js
var 변수를 const와 let으로 수정하는 방법을 배웠습니다. 자바스크립트는 순서대로 실행되며 const에서 오류가 생기면 let으로 수정해야 한다는 것을 한 번더 깨달았고, 전 Gemini 플랫폼을 사용해 var 변수를 const와 let으로 변경할 때 어려움이 생겼는데 간단한 MDN에서 제공하는 연습용 예제인 html을 제작한 틀인 v2.0 시안을 통해 간단한 작업은 var 변수를 const로 변경할 때 자바스크립트 코드의 큰 변화가 생기지 않는다는 걸 알게 되었습니다.

...
function daysAgo(n) { const t = new Date(); t.setDate(t.getDate()-n); return fmtDate(t); };
const nf = (n) => Number(n).toLocaleString('ko-KR');
const AVATAR_COLORS = ['#4F6BF5','#7C3AED','#E5484D','#12A594','#F59E0B','#D6409F','#0EA5E9','#65A30D'];
...
const defaultPosts = [
  ...
  { id:7,  category:'일반', title:'요즘 날씨에 딱 맞는 카페 발견했어요', author:'서연', date:daysAgo(6), views:164, content:'걷다 보니 분위기 좋은 카페를 발견했어요.\n\n창밖으로 정원이 보이는 구조라서 커피 마시며 쉬기 좋더라고요. 위치가 궁금하신 분들은 댓글 주세요!', comments:[] },
...
function renderList(){
   ...
   if (totalCountEl) totalCountEl.textContent = nf(filtered.length);
   ...
   if (postBodyEl) {
      postBodyEl.innerHTML = rows.length ? rows.join('') : `<tr><td colspan="5"><div class="empty">
